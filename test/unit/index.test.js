...
import WechatTokenStrategy from '../../src/index';
...
describe('WechatTokenStrategy:init', () => {
  it('Should properly export Strategy constructor', () => {
    assert.isFunction(WechatTokenStrategy);
  });

  it('Should properly initialize', () => {
    let strategy = new WechatTokenStrategy(STRATEGY_CONFIG, BLANK_FUNCTION);
    assert.equal(strategy.name, 'wechat-token');
  });

  it('Should properly throw exception when options is empty', () => {
    assert.throw(() => new WechatTokenStrategy(), Error);
  });
});

describe('WechatTokenStrategy:authenticate', () => {
  describe('Authenticate without passReqToCallback', () => {
    let strategy;

    before(() => {
      strategy = new WechatTokenStrategy(STRATEGY_CONFIG, (token, tokenSecret, profile, next) => {
  ...
  describe('Authenticate with passReqToCallback', () => {
    ...
    before(() => {
      strategy = new WechatTokenStrategy(Object.assign(STRATEGY_CONFIG, {passReqToCallback: true}), (req, token, tokenSecret, profile, next) => {
  ...
  describe('Failed authentications', () => {
    it('Should properly return error on loadUserProfile', done => {
      let strategy = new WechatTokenStrategy(STRATEGY_CONFIG, (token, tokenSecret, profile, next) => {
        assert.equal(token, 'token');
        assert.equal(tokenSecret, 'token_secret');
        assert.typeOf(profile, 'object');
        assert.typeOf(next, 'function');
...
describe('WechatTokenStrategy:userProfile', () => {
  it('Should properly fetch profile', done => {
    ...
    strategy.userProfile('token', 'tokenSecret', {userId: 'user_id'}, (error, profile) => {
      ...
      assert.equal(profile.provider, 'wechat');
